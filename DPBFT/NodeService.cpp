...
//todo:控制结点的顺序执行(?主节点对消息进行排序),网络传输消息可能丢失、延迟、重复或乱序
// ?是否有必要，存疑，当前主节点最多存六条通知消息，阻塞概率小 -> 没有必要，应该不存在已准备但待处理的消息
void NodeService::onRequest(Request request) {
    ...
    if (getRounds() == 0) {
        setSelectedNodesList(request.selectedNodesList);
    ...
    for (int i = 0; i < count; i++) {
        if (request.selectedNodesList[i] == getId()) {
            ...
            setState(true);
//            string temp = "标识阶段: 投票结点ID: " + to_string(getId()) + " , 主节点编号:" + to_string(getMastNode()) + " , 视图:" +
//                          to_string(getVNum());

            //todo: 结点Id由姐姐点而定，暂时写死 2019年8月6日09点22分
    ...
    if (request.masterId == getId()) {
        ...
        string temp = "pp阶段: 投票结点ID: " + to_string(getId()) + " , 主节点编号:" + to_string(getMastNode())
                      + " , 视图:" + to_string(getVNum()) + " , 序列号:" + to_string(request.seriesNumber)
    ...
    for (int i = 0; i < PrepareList.size(); ++i) {
        if (PrepareList[i] == request.seriesNumber
            ...
            if (countCommit[request.seriesNumber] == getSelectedNodesList().size() / 3 * 2) {
                ...
                //清空所有列表；
                cleanAllList(request.seriesNumber);

                // 调用验证接口，得到验证结果
                // 2019年8月7日 陈晓媛 修改
//                Request rRequest = Quest::toReply(request, getId(), "验证结果");
                Request rRequest = Quest::toReply(request, getId(), to_string(onCountinformation(request)));
...
//2019年8月7日 陈晓媛 添加
void NodeService::onRecord(Request request) {
    //消息类型，发起请求的结点，视图编号，时间戳，摘要，信息，序列号，主节点id，已选列表
    string temp = "记录信息: 投票结点ID: " + to_string(getId()) + " , 主节点编号:" + to_string(request.masterId)
                  + " , 视图:" + to_string(request.vnum) + " , 序列号:" + to_string(request.seriesNumber)
                  + " , 信息:" + request.message + " , 时间戳: " + to_string(request.time);
    writeLog(temp);

    if (request.message != "超时"){
        //清空所有列表；
        cleanAllList(request.seriesNumber);
    }
...
//2019年8月7日 陈晓媛 修改
void NodeService::cleanAllList(int cleanSeriesNumber) {

    deleteMap(countPrepare,cleanSeriesNumber);
    deleteMap(countCommit,cleanSeriesNumber);
    deleteVector(informList, cleanSeriesNumber);
    deleteMap(PrePrepareCandidate, cleanSeriesNumber);
    deleteVector(PrePrepareList, cleanSeriesNumber);
    deleteMap(PrepareCandidate, cleanSeriesNumber);
    deleteVector(PrepareList, cleanSeriesNumber);
    deleteMap(CommitCandidate, cleanSeriesNumber);

}

//2019年8月7日 陈晓媛 添加 删除vector<int>内指定元素 -> cleanAllList用
void NodeService::deleteVector(vector<int> &temp, int num) {
    vector<int>::iterator vectorIterator;
    for (vectorIterator = temp.begin(); vectorIterator != temp.end();) {
        if (*vectorIterator == num) {
            temp.erase(vectorIterator);
            break;
        } else {
            ++vectorIterator;
        }
...
//2019年8月7日 陈晓媛 添加 删除map<int,Request>内指定元素 -> cleanAllList用
void NodeService::deleteMap(map<int, Request> &temp, int num) {
    map<int, Request>::iterator mapIterator;
    for (mapIterator = temp.begin(); mapIterator != temp.end();) {
        if (mapIterator->second.seriesNumber == num) {
            temp.erase(mapIterator);
            break;
        } else {
            ++iter;
        }
...
void NodeService::deleteMap(map<int, int> &temp, int num) {
    map<int, int>::iterator mapIterator;
    for (mapIterator = temp.begin(); mapIterator != temp.end();) {
        if (mapIterator->first == num) {
            temp.erase(mapIterator);
            break;
        } else {
            ++iter;
